...
runtime_path = Path("v0147-crossed-cities-local.js")
source = runtime_path.read_text(encoding="utf-8")
source = source.replace('route-city-crossings-v8-local-south-america', 'route-city-crossings-v9-local-americas')

catalogs = '''  const LOCAL_CATALOGS = Object.freeze({
...
source, n = re.subn(r'  const LOCAL_CATALOGS = Object\.freeze\(\{.*?\n  \}\);', catalogs, source, count=1, flags=re.S)
if n != 1:
    raise SystemExit("LOCAL_CATALOGS block not found")
source = source.replace('catálogos locais sul-americanos habilitadas', 'catálogos locais das Américas habilitadas')
runtime_path.write_text(source, encoding="utf-8")

# Atualiza qualquer expectativa de schema antiga nos testes existentes.
for path in Path("tests").glob("*.test.js"):
    text = path.read_text(encoding="utf-8")
    text = text.replace('route-city-crossings-v8-local-south-america', 'route-city-crossings-v9-local-americas')
    path.write_text(text, encoding="utf-8")

# Teste de integração de catálogo/runtime para todos os países locais.
test_path = Path("tests/v0148-local-cities-ar-uy.test.js")
