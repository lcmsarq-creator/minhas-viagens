...
test("release v0.14.8 invalida o cache do shell e dos módulos", () => {
  ...
  assert.match(authSource, /APP_VERSION=window\.MINHAS_VIAGENS_APP_VERSION\|\|"0\.14\.8"/);
  assert.match(indexSource, /MINHAS_VIAGENS_APP_VERSION = "0\.14\.8"/);
  assert.match(indexSource, /auth\.js\?v=0\.14\.8/);
});
